    ...
    <div id="content">
        ...
        <div class="container mt-0">
            <div class="row breadcrumb-bar">
                <div class="col-md-6">
                    <h3 class="block-title">编辑意见反馈</h3>
                </div>
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item">意见反馈管理</li>
                        <li class="breadcrumb-item active">编辑意见反馈</li>
                    </ol>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    <div class="widget-area-2 lochana-box-shadow">
                        <h3 class="widget-title">意见反馈信息</h3>
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <input id="laoshiId" name="laoshiId" type="hidden">
                                    <div class="form-group col-md-6 yijianfankuiTextDiv">
                                        <label>反馈内容</label>
                                        <textarea style="width: 450px"  id="yijianfankuiText" name="yijianfankuiText" class="form-control"placeholder="反馈内容"></textarea>
                                    </div>
...
<script>
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            httpJson("yijianfankui/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('addyijianfankui');
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                yonghuId: "required",
                laoshiId: "required",
                yijianfankuiText: "required",
                replyText: "required",
            },
            messages: {
                yonghuId: "用户不能为空",
                laoshiId: "老师不能为空",
                yijianfankuiText: "反馈内容不能为空",
                replyText: "回复内容不能为空",
            }
        }).form();
    }
    ...
    // 获取当前详情
    function getDetails() {
        var addyijianfankui = window.sessionStorage.getItem("addyijianfankui");
        if (addyijianfankui != null && addyijianfankui != "" && addyijianfankui != "null") {
            //注册表单验证
            $(validform());
            ...
            window.sessionStorage.removeItem('updateId');
            http("yijianfankui/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
    ...
    <!--  当前表的数据回显  -->
        ...
        $("#yijianfankuiText").val(ruleForm.yijianfankuiText);
    ...
    function readonly() {
        if (window.sessionStorage.getItem('role') == '管理员') {
            $('#yijianfankuiText').attr('readonly', 'readonly');
            ...
            //设置留言或者评价不能修改
            $('#yijianfankuiText').attr('readonly', 'readonly');
            var yijianfankuiTextUeditor = UE.getEditor('yijianfankuiTextEditor');
            yijianfankuiTextUeditor.ready(function () {
                yijianfankuiTextUeditor.setDisabled('fullscreen');
            });
        }
        else{
            // alert("未知情况.......");
